...
一句话：把「生成」做成一条**可观测、可中断、可追溯**的流水线，而不是一个聊天框。
...
| 测试规模 | 数据库 102 例 · backend 160 例 · ai-engine 207 例（其中 195 例纯内存可跑）· 三个前端各有独立用例集 |
...
| 套件 | 例数 | 内容 |
|---|---|---|
| 数据库 | 102 | 权限矩阵 88 + 红线 6 + 迁移/约束 8 |
| backend | 160 | 认证 / RBAC / CAS / SSE / 一致性契约 |
| ai-engine | 207 | 图 / Agent / 工具 / 可靠性（195 例纯内存可跑，12 例需容器化 PG + Redis） |
...
因此，如果你在代码注释或历史讨论里看到 `backend-api/README.md`、`infra/docs/deploy.md` 之类的引用，
它们指向的是那套内部文档 —— 对外发布产物里只有这一个 `README.md`。
