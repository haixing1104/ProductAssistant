#!/usr/bin/env bash
# =============================================================================
# ProductAssistant | scripts/test-landing.sh
# 用途    : 作品集宣传页（portfolio/）用例运行器 —— 与 test-frontend.sh 同一套收尾策略：
#           本沙箱（Node 24 + vitest 5 + jsdom）下全量跑会在用例通过后卡在 teardown，
#           「退出码」并不是可信信号，**以每个文件的 ✓/× 行为准**。
# 用法    : ./scripts/test-landing.sh                     # 全量（portfolio/src/__tests__/*）
#           ./scripts/test-landing.sh src/__tests__/projects.test.ts
#           PA_TEST_TIMEOUT=300 ./scripts/test-landing.sh
# 退出码  : 0 = 所有已发现文件都 ✓ 且无 ×；1 = 有失败/超时/文件数不符。
# 说明    : 本模块与 frontend/ 是两套独立的 vitest 工程（依赖也不同：Tailwind v4 只装在
#           portfolio/），所以单独一个运行器，互不影响。
# =============================================================================
set -uo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="${PAD_LOG_DIR:-${TMPDIR:-/tmp}}"
LOG="$LOG_DIR/landing-tests.log"
CLEAN="$LOG.clean"
...
cd "$ROOT/portfolio" || { err "找不到 portfolio/ 目录"; exit 1; }
mkdir -p "$LOG_DIR"
